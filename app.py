...
with col_main:
    ...
    with st.expander("🌍 미국 시장 실시간 흐름 (보조지표 + 레이어)", expanded=True):
        ...
        verdict = compute_market_verdict_scores(ov, bigtech_list_len=len(getattr(market, "BIGTECH_LIST", []) or [1]))
        ...
        bt_score = (ov.get("bigtech", {}) or {}).get("score", 0)
        bt_items = (ov.get("bigtech", {}) or {}).get("items", [])
        ...
        sec_score = (ov.get("sector", {}) or {}).get("score", 0)
        sec_items = (ov.get("sector", {}) or {}).get("items", [])
        st.markdown('<div class="card-soft">', unsafe_allow_html=True)
    ...
    with st.expander("🛰 신규 진입 스캐너 (간단 버전)", expanded=False):
        ...
        if scan_click:
            with st.spinner("신규 진입 후보 종목 스캔 중..."):
                scan_mkt_score, scan_list = scan_new_entry_candidates(
                    cfg=cfg,
                    max_results=8,
                    get_us_market_overview_fn=get_us_market_overview,
                    compute_market_score_fn=compute_market_score,
    ...
    levels_dict = {"buy_low": buy_low, "buy_high": buy_high, "tp0": tp0, "tp1": tp1, "tp2": tp2, "sl0": sl0, "sl1": sl1}

    state_name, action_text, recover_level, phase = compute_state_and_action(
        holding_type=holding_type, price_now=price_now, avg_price=avg_price, levels=levels_dict, last_row=last
    )
    ...
    score_mkt2, label_mkt2, detail_mkt2 = compute_market_score(ov)
    alerts = analysis.build_risk_alerts(score_mkt2, last, gap_pct, atr14, price_move_abs)
    ...
    st.write(f"- 공포·탐욕지수(FGI, CNN): **{fgi:.1f}**" if fgi is not None else "- 공포·탐욕지수(FGI): 조회 실패 → 시장심리는 제외하고 지표만 사용")
    ...
    with col_vp1:
        if vp_levels:
            ...
            for lv in vp_levels:
                st.write(f"- **{lv['low']:.2f} ~ {lv['high']:.2f} USD** (중심 {lv['mid']:.2f}) – 거래량 많은 구간")
        else:
    ...
    if cache_key is not None:
        st.button(btn_label, key=f"ai_btn_{cache_key}", on_click=request_ai_generation, args=(cache_key,))
    else:
    ...
    if st.session_state.get("ai_request", False) and st.session_state.get("ai_request_key") == cache_key:
        ...
        extra_notes = [
            f"시장점수: {score_mkt2} / label: {label_mkt2}",
            f"현재상태: {state_name}",
            f"행동지침: {action_text}",
        ...
        if structure_broken:
            levels_for_ai = {"sl0": levels_dict.get("sl0"), "sl1": levels_dict.get("sl1"), "recover": recover_level}
        ...
        with st.spinner("AI 해석 생성 중..."):
            parsed, err = ai_summarize_and_explain(
                ...
                holding_type=holding_type,
                mode_name=mode_name,
                market_label=label_mkt2,
                market_detail=detail_mkt2,
                ...
                model_name=st.session_state.get("ai_model_name", "gpt-4o-mini"),
            )
